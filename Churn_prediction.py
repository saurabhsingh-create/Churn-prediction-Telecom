...
os.chdir(r'C:\Users\saurabhsingh\Downloads\Churn_prediction\Input_data')
...
#7043 rows 21 col and no missing value
...
#SVC performs better along with kernel svm

#Plot for AUC
#using Area under ROC Curve:
# Logistic and SVC seems to perform well
fig = plt.figure(figsize=(15, 7))
ax = fig.add_subplot(111)
...
#Now optimie the parameters and finalize metrics for evaluation

#Best paramters for KNN
...
fig = plt.figure(figsize=(15,7)) 
plt.plot(range(1,25),knn_score, color = '#ec838a')   
...
#Similarly we can optimal paramters for random forest
...
#highest accuracy score when its n_estimators = 72.
# here false negatives are more costly then false positives

# So we need to look for precisin recall and f1 score.
...
#fitting a svc classifier
classifier =  SVC(kernel = 'linear', random_state=0)
classifier.fit(X_train, y_train)
#predict
y_pred = classifier.predict(X_test)
# get precsion, recall,  accuracy f1 and f beta score
acc = accuracy_score(y_test, y_pred)
prec = precision_score(y_test, y_pred)
rec = recall_score(y_test, y_pred)
f1 = f1_score(y_test, y_pred)
f2 = fbeta_score(y_true, y_pred, beta = 2.0)  #f beta is harmonic mean of precsion and recall
...
#From the 2nd iteration, we can definitely conclude that logistic regression is an 
# optimal model of choice for the given dataset as it has relatively the highest combination 
# of precision, recall and F2 scores; giving most number of correct positive predictions 
# while minimizing the false negatives. Hence, let's try to use Logistic Regression 
# and evaluate its performance in the forthcoming sections.

#final model and kfold cv
...
#quantile cuts
final_results ['Ranking'] = pd.qcut(final_results['propensity_to_convert(%)'].
                                    rank(method = 'first'),10,labels=range(10,0,-1))
...
#################### Final Steps for modelling ###########################

# 1. Read data
# 2. Treat missing values and outliers by mode or mean of columns and check for nulls and missing
# 3. Label encode with 2 unique value columns
# 4. Convert numeric columns
# 5. Categorcial encoding for more than 3 catgories
# 6. Visualize everything against churn and plot vif and correlations plots. 
# 7. remove columns high multicollinearity.
# 8. fit logistic, linear svm, kernal svm, knn, decision tree, random forest
# 9. Get best based on precion, recall and f1 score as false negatives are imp
# 10. choose logistic and get best paramters
# 11. use kfold to get variance of accuracies
# 12. get best parameters using grid search of multiple layers
# 13. calculate predictions, probablities, and ranking
# 14. make confusion matrix and visualize
# 15. dump model for model deployment